...
func TestAnInfrastructureFailureIsRecordedButNotAsAnAttempt(t *testing.T) {
	...
	_, err := implstep.New(b, b, b, r, nil).Do(context.Background(), testCard(), res(1))
	if err == nil {
...
func TestARetryReceivesEvidenceNotMonologue(t *testing.T) {
	...
	if _, err := implstep.New(b, b, b, r, nil).Do(context.Background(), testCard(), res(2)); err != nil {
		t.Fatal(err)
	}
...
func TestTheTaskForbidsChangingTheTests(t *testing.T) {
	...
	if _, err := implstep.New(b, b, b, r, nil).Do(context.Background(), testCard(), res(1)); err != nil {
		t.Fatal(err)
	}
